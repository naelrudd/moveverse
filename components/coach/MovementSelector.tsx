...
/** World theme configs */
const WORLD_THEMES: Record<string, { bg: string; border: string; accent: string; levelActive: string; levelInactive: string; headerBg: string }> = {
  'pulau-naga': {
    bg: 'bg-gradient-to-br from-green-50 to-emerald-50',
    border: 'border-green-300',
    accent: 'text-green-700',
    levelActive: 'bg-green-600 text-white border-green-500 ring-2 ring-green-300',
  ...
  'gunung-elang': {
    ...
    levelActive: 'bg-purple-600 text-white border-purple-500 ring-2 ring-purple-300',
    levelInactive: 'bg-white text-purple-700 border-purple-200 hover:bg-purple-50',
...
const DEFAULT_THEME = {
  ...
  border: 'border-sky-300',
  accent: 'text-sky-700',
  levelActive: 'gradient-grass text-white shadow-pop scale-110 border-green-500 ring-2 ring-green-300',
  levelInactive: 'bg-white text-gray-600 border-gray-200 hover:border-green-300 hover:bg-green-50 hover:scale-105',
  headerBg: 'bg-sky-600',
};
...
export function MovementSelector({ activity, level, onActivityChange, onLevelChange, worldId }: MovementSelectorProps) {
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    if (dx < 0 && idx < ACTIVITIES.length - 1) onActivityChange(ACTIVITIES[idx + 1].id);
