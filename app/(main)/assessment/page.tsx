...
import {
  useLiveCoachEngine,
  type SessionResult,
} from "@/hooks/useLiveCoachEngine";
import {
  type MovementType,
  movementToPhysicalLiteracy,
    ...
    <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-green-200 animate-pop-in relative overflow-hidden">
      ...
      <div className="text-center relative z-10">
        ...
        <p className="text-sm font-bold text-muted-foreground mb-4">
          {result.activity} Lv.{result.level} • {result.duration.toFixed(0)}{" "}
          detik
        </p>
            ...
            <div className="flex gap-2 flex-wrap justify-center">
              {newBadges.map((b) => {
                ...
                return (
                  <div
                    key={b}
                    className="bg-amber-50 border-2 border-amber-300 rounded-xl px-3 py-2 text-center animate-pop-in"
                  >
        ...
        <div className="bg-gradient-to-r from-amber-100 to-yellow-100 rounded-xl p-3 border-2 border-amber-300 mb-4">
          <span className="text-sm font-extrabold text-amber-800">
            ⭐ +
            {Math.round(
              result.avgScore * 0.5 + result.reps * 2 + result.level * 3,
            )}{" "}
            XP
          </span>
          <span className="text-xs font-bold text-amber-600 ml-2">
            ({result.reps} rep × 2 + Lv.{result.level} × 3 + avgScore × 0.5)
          </span>
        </div>

        {/* Mini Radar Chart (Physical Literacy) */}
        <div className="bg-white rounded-xl p-3 border-2 border-sky-200 mb-4">
          <div className="text-xs font-extrabold text-sky-700 mb-2">
            🎯 Kontribusi Literasi Fisik
          </div>
          <div className="flex justify-center">
            <svg width="140" height="140" viewBox="0 0 200 200">
              {(() => {
                const metrics = movementToPhysicalLiteracy(
                  result.activity,
                  result.avgScore,
                );
        ...
        <div className="flex items-start gap-3 text-left bg-primary/5 rounded-2xl p-4 border-2 border-primary/10 mb-4">
          ...
          <div className="flex-1">
            <div className="text-[10px] font-extrabold text-primary mb-0.5">
              ✨ MOVA berkata
            </div>
...
function AssessmentContent() {
  ...
  const classes = useQuery(
    ...
    role === "teacher" && userData?.schoolId
    ...
    (result: SessionResult) => {
      ...
      if (audioEnabled) {
        try {
          ...
          osc.frequency.value =
            result.avgScore >= 80 ? 523 : result.avgScore >= 60 ? 440 : 349;
          ...
          .then((res) => {
            if (res?.newBadges?.length) {
              setSessionResult((prev) =>
                prev ? { ...prev, newBadges: res.newBadges } : prev,
              );
            }
          })
          .catch(console.error);
  ...
  return (
    <div
      className={`min-h-screen relative ${coach.isFullScreen ? "" : "bg-theme-tiger"}`}
    >
        ...
        createPortal(
          <div className="fixed inset-0 z-[9999] bg-black flex flex-col">
            <LivePoseCoach
              ...
              canvasRef={coach.canvasRef}
              levelConfig={coach.levelConfig}
            />
          </div>,
          document.body,
        )}
            ...
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-amber-500 to-green-500 flex items-center justify-center gap-2 sm:gap-3">
              ...
              {role === "teacher"
                ...
                : role === "parent"
                  ...
                  : "Tunjukkan Gerakanmu!"}
              <span
                className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 p-1 shadow-pop inline-block animate-float overflow-hidden flex-shrink-0"
          ...
          {activityObjective && (
            <div className="mb-8 bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 rounded-[2rem] border-4 border-green-300 shadow-pop p-5 animate-pop-in relative overflow-hidden">
              ...
              <div className="flex items-start gap-4 relative z-10">
                <span className="text-4xl animate-wiggle flex-shrink-0">
                  🌍
                </span>
                <div className="flex-1">
                  ...
                  <div className="font-extrabold text-lg text-green-800">
                    {worldName ? `Dunia ${worldName}` : "Aktivitas"} —{" "}
                    {activityName}
                  </div>
                ...
                <span className="text-3xl animate-bounce-sm flex-shrink-0">
                  ✨
                </span>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <div className="grid lg:grid-cols-5 gap-4 relative">
                ...
                <div className="lg:col-span-2 space-y-4 relative">
                    ...
                    <div className="flex items-center gap-2 relative z-10">
                      ...
                      <div className="bg-primary/10 rounded-xl rounded-tl-none px-3 py-2 flex-1 shadow-soft">
                        ...
                        <p className="text-[11px] font-bold leading-snug">
                          {role === "teacher" ? (
                            <>
                              Rekaman untuk {targetName ?? "..."}! 🎬 Pilih
                              gerakan & mulai! 💪
                            </>
                          ) : (
                            <>
                      ...
                      {showCalibration && (
                        <div className="mt-3 space-y-2 text-xs">
                          <p className="text-muted-foreground font-medium">
                            Atur threshold untuk {activity} Lv.{level}:
                          </p>
                          {[
                            {
                              key: `${activity}_minScore`,
                              label: "Min Score",
                              def: [60, 70, 80, 85, 90][level - 1],
                            },
                            {
                              key: `${activity}_holdSec`,
                              label: "Hold (detik)",
                              def: [3, 5, 5, 3, 2][level - 1],
                            },
                          ].map(({ key, label, def }) => (
                  ...
                  {((role as string) === "teacher" ||
                    (role as string) === "parent") &&
                    liveSessionData && (
                      <div className="bg-white rounded-2xl p-4 shadow-soft border-2 border-sky-200">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span className="text-xl">
                            {liveSessionData.avatar}
                          </span>
                          <div className="flex-1">
                            <div className="font-bold">
                              Lv.{liveSessionData.level} • {liveSessionData.xp}{" "}
                              XP
                            </div>
                          </div>
                        </div>
                            ...
                            {(
                              ...
                              <div key={k} className="bg-sky-50 rounded-lg p-1">
                                <div className="text-[10px] font-bold text-sky-700">
                                  {liveSessionData.physicalLiteracy![k]}
                                </div>
                                <div className="text-[8px] text-sky-500">
