...
const dummyLeaderboard: { _id?: string; rank: number; name: string; avatar: string; level: number; xp: number; badges?: string[]; trend?: string }[] = [
  ...
  { rank: 2, name: 'Nina', avatar: '👧', level: 4, xp: 3800, trend: 'up' },
  { rank: 3, name: 'Rian', avatar: '🧒', level: 3, xp: 3100, trend: 'stable' },
  { rank: 4, name: 'Putri', avatar: '👧', level: 3, xp: 2900, trend: 'up' },
  { rank: 5, name: 'Adi', avatar: '🧒', level: 3, xp: 2450, trend: 'down' },
  { rank: 6, name: 'Budi', avatar: '🧒', level: 2, xp: 2100, trend: 'up' },
  { rank: 7, name: 'Joko', avatar: '🧒', level: 2, xp: 1900, trend: 'stable' },
  { rank: 8, name: 'Sari', avatar: '👧', level: 2, xp: 1800, trend: 'down' },
];
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="flex gap-3 mt-3 flex-wrap">
          ...
          <div className="ml-auto flex gap-1">
            <button
              ...
              className={`px-3 py-2 rounded-full text-sm font-bold ${sortBy === 'xp' ? 'bg-blue-500 text-white' : 'bg-muted'}`}
            >
              XP
            </button>
            <button
              onClick={() => setSortBy('level')}
              className={`px-3 py-2 rounded-full text-sm font-bold ${sortBy === 'level' ? 'bg-purple-500 text-white' : 'bg-muted'}`}
            >
              Level
            </button>
          </div>
      ...
      {!selectedClassId ? (
        <div className="bg-white rounded-3xl p-8 shadow-soft text-center">
          ...
          <p className="font-bold text-muted-foreground">Pilih kelas untuk melihat papan skor</p>
        </div>
      ) : (
        <>
          {/* Podium */}
          {sorted.length >= 3 && (
            <div className="grid grid-cols-3 gap-3">
              {[1, 0, 2].map((idx) => {
                ...
                return (
                  <div key={s._id} className={`bg-gradient-to-b ${colors[idx]} rounded-3xl p-4 flex flex-col items-center justify-end pb-4 text-white shadow-soft`} style={{ minHeight: '130px' }}>
                    ...
                    <div className="text-xs font-bold mt-1 bg-white/30 rounded-full px-2 py-0.5">
                      {sortBy === 'xp' ? `${s.xp.toLocaleString()} XP` : `Lv ${s.level}`}
                    </div>
                  </div>
                );
              })}
            ...
            {sorted.map((s, i) => (
              ...
              >
                ...
                <div className="text-center">
                  <div className="font-extrabold text-lg">{s.xp.toLocaleString()}</div>
                  <div className="text-[10px] text-muted-foreground">Total XP</div>
                </div>
                <div className="text-center">
                  <div className="font-extrabold">{s.badges?.length ?? 0}/6</div>
                  <div className="text-[10px] text-muted-foreground">Lencana</div>
                </div>
              </div>
            ))}
          </div>
