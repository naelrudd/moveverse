...
export default function ParentDashboard() {
  ...
  const [extraChildren, setExtraChildren] = useState<ChildItem[]>([]);
  ...
  const sideQuests = useQuery(
    ...
    activeChildId && userData?._id
      ? { parentId: userData._id, childId: activeChildId as Id<"users"> }
      : "skip",
  );
  ...
  const getSportRecommendations = () => {
    const kaki =
      (motorikData.find((m) => m.skill === "Kelincahan")?.value ?? 0) +
      (motorikData.find((m) => m.skill === "Kekuatan")?.value ?? 0);
    const tangan =
      (motorikData.find((m) => m.skill === "Koordinasi")?.value ?? 0) +
      (motorikData.find((m) => m.skill === "Fleksibilitas")?.value ?? 0);
    const badan =
      motorikData.find((m) => m.skill === "Keseimbangan")?.value ?? 0;
    ...
    if (kaki >= tangan) {
      recs.push({
        name: "Lari Ringan",
        icon: "🏃",
        reason: "Kelincahan kaki sudah bagus, tambah latihan lari",
        kategori: "Kaki",
      });
      recs.push({
        name: "Skipping",
        icon: "🪢",
        reason: "Koordinasi kaki & lompat",
        kategori: "Kaki",
      });
      recs.push({
        name: "Sepak Bola",
        icon: "⚽",
        reason: "Teknik tendangan",
        kategori: "Kaki",
      });
    } else {
      recs.push({
        name: "Bulu Tangkis",
        icon: "🏸",
        reason: "Koordinasi tangan sudah bagus",
        kategori: "Tangan",
      });
      recs.push({
        name: "Volli",
        icon: "🏐",
        reason: "Kekuatan tangan & lompat",
        kategori: "Tangan",
      });
      recs.push({
        name: "Lempar Tangkap",
        icon: "🤾",
        reason: "Akurasi lemparan",
        kategori: "Tangan",
      });
    }
    if (badan >= 65) {
      recs.push({
        name: "Senam Ringan",
        icon: "🤸",
        reason: "Keseimbangan tubuh baik",
        kategori: "Badan",
      });
    } else {
      recs.push({
  ...
  return (
    <div
      className="max-w-7xl mx-auto px-4 py-8 space-y-8 bg-theme-dragon min-h-screen"
    ...
    >
      ...
      <section className="relative rounded-[2rem] overflow-hidden shadow-pop border-4 border-white animate-pop-in">
        ...
        <div
          ...
          style={{
            background:
              "oklch(0.82 0.12 310) 0%, oklch(0.78 0.14 330) 25%, oklch(0.75 0.16 350) 50%, oklch(0.78 0.15 25) 75%, oklch(0.82 0.14 55) 100%",
        ...
        <div className="relative z-10 p-6 md:p-8">
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 mb-6">
            ...
            <div className="relative shrink-0">
              <div className="w-32 h-32 relative rounded-full bg-linear-to-br from-amber-400 to-orange-500 p-1 shadow-soft overflow-hidden animate-dance-slow border-4 border-white">
                <Image
                  src="/mova-hero.png"
                  alt="MOVA"
                  fill
              ...
              <div className="absolute -bottom-4 -right-6">
                <Image
                  ...
                  width={80}
                  height={80}
                  className="h-20 w-auto animate-float relative"
                  style={{ animationDelay: "0.5s" }}
                />
                <span
                  className="absolute top-0 left-2 text-xs animate-sparkle text-yellow-300"
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <p
                className="text-lg font-bold text-foreground/70 mt-1"
                style={{ fontFamily: "Nunito, sans-serif" }}
              >
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            l: "Level",
            v: `Level ${activeChild?.level ?? 0}`,
            icon: "🏆",
            gradient: "from-violet-500 to-purple-600",
          },
          {
            l: "XP Total",
            v: (activeChild?.xp ?? 0).toLocaleString(),
            icon: "⚡",
            gradient: "from-orange-400 to-red-500",
          },
          {
            l: "Badge",
            v: `${activeChild?.badges?.length ?? 0}/${ALL_ACTIVITIES.length}`,
            icon: "🏅",
          ...
          >
            ...
            {/* Sparkle */}
            <span
              className="absolute top-2 right-3 text-yellow-200 text-xs animate-sparkle"
              style={{ animationDelay: `${i * 0.3}s` }}
            >
              ✦
            </span>
            <div className="relative z-10 flex items-center gap-3">
              <span className="text-3xl drop-shadow-sm group-hover:animate-bounce-sm">
                {s.icon}
              </span>
              <div>
      ...
      <section>
        <div className="rounded-[2rem] overflow-hidden shadow-pop mb-6 border-4 border-white">
          ...
          >
            <div className="absolute inset-0 bg-linear-to-r from-purple-600/80 via-pink-500/70 to-orange-500/60" />
            <div className="relative flex items-center gap-4">
              ...
              <div>
                <h3
                  className="font-extrabold text-2xl text-white drop-shadow-md"
                  style={{ fontFamily: "Fredoka, sans-serif" }}
                >
                  🌍 Dunia Petualangan Anak
                </h3>
                <p className="text-sm text-white/90 font-bold">
                  3 dunia penuh seru! Lihat perkembangan buah hati 💖
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {worlds.map((w) => {
            const earnedCount = activeChild?.badges
              ? w.activities.filter((a) =>
                  activeChild?.badges?.includes(a.badgeId),
                ).length
              : 0;
            ...
            return (
              <Link key={w.id} href={`/worlds/${w.id}`} className="block group">
                ...
                >
                  ...
                  />
                  <div
                    className={`absolute inset-0 ${w.gradient} opacity-70`}
                  />
                  ...
                  <div className="absolute top-4 left-5 z-10">
                    <span
                      className="text-5xl drop-shadow-md animate-float"
                  ...
                  <div className="absolute bottom-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Image
                      ...
                      width={40}
                      height={40}
                      className="w-10 h-10 rounded-full border-2 border-white shadow-soft animate-wobble"
                    />
                  </div>

                  <div className="relative p-5 flex flex-col justify-end min-h-85 z-10">
                    <div className="mt-auto">
                      ...
                      <p className="text-xs opacity-90 mt-0.5 drop-shadow-sm font-bold">
                        {w.tagline}
                      </p>
                      ...
                      <div className="mt-3">
                        <div className="h-3 bg-black/20 rounded-full overflow-hidden backdrop-blur-sm">
                          <div
                            className="h-full bg-white rounded-full transition-all shadow-inner"
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-1.5 mt-3">
                        {w.activities.map((a) => {
                          const earned = activeChild?.badges?.includes(
                            a.badgeId,
                          );
                          return (
                            <div
      ...
      <section>
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-white relative overflow-hidden">
            <span className="absolute top-4 right-4 text-yellow-300 text-sm animate-sparkle">
              ✦
            </span>
            <span
              className="absolute bottom-4 left-4 text-pink-300 text-xs animate-sparkle"
              style={{ animationDelay: "1s" }}
            >
              ✦
            </span>
            <h3
              className="font-extrabold text-xl mb-4 flex items-center gap-2"
              style={{ fontFamily: "Fredoka, sans-serif" }}
            >
            ...
            <div className="flex gap-4 justify-center text-xs font-bold mt-2">
              ...
              <span className="flex items-center gap-1">
                <span
                  className="w-3 h-0.5 bg-slate-400 inline-block"
              ...
              {motorikData.map((m) => (
                <div key={m.skill} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex-1 h-3 bg-purple-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-linear-to-r from-purple-400 to-pink-500 rounded-full transition-all"
                      style={{ width: `${m.value}%` }}
                    />
                  </div>
                  <span className="font-bold w-8 text-right">{m.value}</span>
                  <span
                    className={`text-xs font-bold ${m.value >= m.prev ? "text-green-500" : "text-red-400"}`}
                  >
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-white relative overflow-hidden">
            <span
              className="absolute top-4 right-4 text-blue-300 text-sm animate-sparkle"
            ...
            <div className="space-y-3">
              {[
                ...
                <div
                  key={i}
                  className="flex items-center justify-between p-4 bg-linear-to-r from-purple-50 to-pink-50 rounded-2xl hover:from-purple-100 hover:to-pink-100 transition-all cursor-pointer border-2 border-transparent hover:border-purple-200 hover:shadow-soft"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-linear-to-br from-purple-400 to-pink-500 flex items-center justify-center text-white text-lg shadow-soft">
                      🎥
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-sm font-bold ${rec.score >= 80 ? "text-green-500" : rec.score >= 65 ? "text-amber-500" : "text-red-400"}`}
                    >
                      {rec.score}%
                    </span>
      ...
      <section>
        <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-white relative overflow-hidden">
          <span className="absolute top-5 right-5 text-yellow-300 text-sm animate-sparkle">
            ✦
          </span>
          <span
            className="absolute bottom-5 left-5 text-pink-300 text-xs animate-sparkle"
          ...
          <div className="flex items-center gap-3 mb-5 bg-orange-50 rounded-2xl p-3 border-2 border-orange-200">
            <Image
              ...
              width={32}
              height={32}
              className="w-8 h-8 rounded-full animate-wobble shrink-0"
            />
            <span className="text-xs font-bold text-orange-700">
              💡 MOVA kasih rekomendasi kegiatan seru buat{" "}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold px-3 py-1 rounded-full gradient-magic text-white shadow-soft">
              Berdasarkan Stat ✨
            </span>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {getSportRecommendations().map((r) => (
              <div
                key={r.name}
                className="bg-linear-to-br from-orange-50 to-pink-50 rounded-3xl p-5 hover:shadow-pop transition-all border-2 border-transparent hover:border-orange-200 hover:scale-105 group cursor-pointer"
              >
                <div className="text-4xl mb-2 group-hover:animate-bounce-sm">
                  {r.icon}
                </div>
                <div
                ...
                <div className="text-[10px] font-bold text-orange-500 mt-1 px-2 py-0.5 rounded-full bg-orange-100 inline-block">
                  {r.kategori}
                </div>
          ...
          <p className="text-xs text-muted-foreground mt-3 font-semibold">
            * Rekomendasi ini hanya saran kegiatan tambahan di rumah, bukan
            bagian dari poin peserta didik 🏡
          </p>
        </div>
      </section>
      ...
      <section>
        <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-white relative overflow-hidden">
          <span
            className="absolute top-5 right-5 text-yellow-300 text-sm animate-sparkle"
            style={{ animationDelay: "0.4s" }}
          >
            ✦
          </span>
          <span
            className="absolute bottom-5 left-5 text-green-300 text-xs animate-sparkle"
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold px-3 py-1 rounded-full gradient-sunset text-white shadow-soft">
              Tambahan Poin ⚡
            </span>
          </div>
          <p className="text-sm text-muted-foreground mb-4 font-semibold">
            ...
            nambah poin dengan tugas tambahan di rumah! 💪
          </p>

          <div className="space-y-3">
            {(sideQuests ?? []).length === 0 && (
              <div className="p-8 text-center bg-linear-to-br from-yellow-50 to-orange-50 rounded-3xl border-2 border-dashed border-yellow-300">
                <Image
                  src="/mova-hero.png"
                  alt="MOVA"
                  width={64}
                  height={64}
                  className="w-16 h-16 mx-auto rounded-full animate-float mb-3"
                />
                <div
                  className="font-extrabold text-sm text-orange-700"
                  style={{ fontFamily: "Fredoka, sans-serif" }}
                >
                  Belum ada side quest! 📋
                </div>
                <div className="text-xs text-muted-foreground font-semibold mt-1">
                  Yuk buat tugas baru buat {activeChild?.name || "anak"}! ✨
                </div>
              </div>
            )}
              ...
              <div
                ...
                className={`p-4 rounded-2xl flex items-center gap-3 border-2 transition-all ${
                  q.completed
                    ...
                    : "bg-linear-to-r from-orange-50 to-pink-50 border-orange-100 hover:border-orange-200 hover:shadow-soft"
                }`}
              >
                <div
                  className={`w-11 h-11 rounded-full flex items-center justify-center text-lg shadow-soft ${q.completed ? "gradient-grass text-white" : "bg-white border-2 border-orange-200"}`}
                >
                  {q.completed ? "✅" : q.icon}
                </div>
                <div className="flex-1">
                  <div
                    className={`font-extrabold text-sm ${q.completed ? "line-through text-muted-foreground" : ""}`}
                    style={{ fontFamily: "Fredoka, sans-serif" }}
                  >
                    {q.title}
                  </div>
          ...
          {showQuestForm && activeChildId && (
            <div className="mt-4 p-5 bg-linear-to-br from-purple-50 to-pink-50 rounded-3xl space-y-3 animate-pop-in border-2 border-purple-200 shadow-soft">
              <input
                value={newQuestTitle}
                ...
                <button
                  onClick={async () => {
                    ...
                    await createSideQuest({
                      parentId: userData._id,
                      childId: activeChildId as Id<"users">,
      ...
      <section>
        <div className="bg-white rounded-[2rem] p-6 shadow-pop border-4 border-white relative overflow-hidden">
          <span className="absolute top-4 right-4 text-blue-300 text-sm animate-sparkle">
            ✦
          </span>
          <h3
            className="font-extrabold text-xl mb-4 flex items-center gap-2"
          ...
          <div className="flex gap-4 justify-center text-xs font-bold mt-2">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-purple-500" /> Di Sekolah
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-orange-500" /> Di Rumah
            </span>
          </div>
        </div>
      </section>
      ...
      <div className="flex justify-center pb-8">
        <div className="bg-linear-to-r from-purple-100 via-pink-100 to-orange-100 border-2 border-purple-200 rounded-3xl px-6 py-4 shadow-soft inline-flex items-center gap-3 animate-wobble max-w-md text-center">
          <Image
            src="/mova-hero.png"
            alt="MOVA"
            width={40}
            height={40}
            className="w-10 h-10 rounded-full animate-dance-slow shrink-0"
          />
          <div>
            <div
            ...
            <div className="text-xs font-bold text-purple-500 mt-0.5">
              Terus pantau perkembangan {activeChild?.name || "anak"} ya! Setiap
